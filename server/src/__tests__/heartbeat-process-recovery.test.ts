...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not reap active adapter executions started by the heartbeat service instance", async () => {
    ...
    const result = await executorHeartbeat.reapOrphanedRuns({ staleThresholdMs: 1 });
    expect(result).toEqual({ reaped: 0, runIds: [] });

    const activeRun = await executorHeartbeat.getRun(runId);
  ...
  it("does not escalate exhausted successful handoff for permanent watcher issues", async () => {
  ...
  it("does not re-enqueue continuation for stranded standing in-progress work", async () => {
