...
import { and, asc, desc, eq, getTableColumns, gt, gte, inArray, isNull, lt, lte, not, notInArray, or, sql } from "drizzle-orm";
  ...
  return {
    ...
    getExecutionCapSummary: async () => {
      const [globalRunning, globalQueued, premiumRunning, pausedWakeBackoffRows] = await Promise.all([
        ...
        listPremiumRunningRuns().then((runs) => runs.length),
        db
          .select({
            agentId: heartbeatRuns.agentId,
