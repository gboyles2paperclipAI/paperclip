...
export function deriveRuntimeControls(input: {
  ...
  const roleAllowsSchedulerWork = role === "primary";
  const primaryOnly = role === "primary";
  const heartbeatSchedulerEnabled = roleAllowsSchedulerWork && (heartbeatSchedulerExplicit ?? true);
  ...
  const pluginSchedulerEnabled = roleAllowsSchedulerWork;
  const pluginWorkersEnabled = roleAllowsSchedulerWork;
  const pluginAutoInstallEnabled = primaryOnly;
