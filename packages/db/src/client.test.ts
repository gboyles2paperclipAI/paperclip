...
describeEmbeddedPostgres("resetPostgresDatabase", () => {
  it(
    "recreates an existing database so stale tables are removed",
    async () => {
    ...
    // Embedded Postgres startup pushed this integration case to 5,022 ms on paperclip01.
    15_000,
  );
});
...
describeEmbeddedPostgres("applyPendingMigrations", () => {
  ...
  it(
    ...
    async () => {
      ...
      const builtInResourcesHash = await migrationHash(
        "0142_built_in_managed_resources.sql",
      );
      ...
      expect(pendingState).toMatchObject({
        status: "needsMigrations",
        pendingMigrations: ["0142_built_in_managed_resources.sql"],
        reason: "pending-migrations",
      });
  ...
  it(
    "replays migration 0136 without bumping issue updated_at for inbox archives",
    async () => {
      const connectionString = await createTempDatabase();
  ...
  it(
    "replays migration 0137 to repair updated_at sweeps and no-op when clean",
    async () => {
      const connectionString = await createTempDatabase();
