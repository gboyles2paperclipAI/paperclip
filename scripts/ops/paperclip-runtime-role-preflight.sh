...
if [[ -n "$production_db_host" && -n "${DATABASE_URL:-}" ]]; then
  ...
  if [[ "$db_host" == "$production_db_host" && -z "$role" ]]; then
...
const passiveRoles = new Set(["api-only", "scheduler-only", "staged"]);
...
console.log(JSON.stringify({
  ...
  pluginAutoInstallEnabled: runtime.pluginAutoInstallEnabled,
